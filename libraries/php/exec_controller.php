<?php
  ...
  class Commit{
	...
	if($action=="set_material_downloadable"){
	  $this->setDownloadable($target);
	}
     }
	 ...
	 if($q_model->del_test()){
       ...
       if ($target == 'student_course'){
	 require_once "../../instructor_module/class_models/student_roster_model.php";
	 $SRM = new IMStudentRoster();
	 $SRM->unenroll();
     ...
     function setDownloadable($target){
       if($target=="lect_mat"){
	 require_once "../../instructor_module/class_models/im_syllabus.php";
	 $sylab = new IMSyllabus();
	 $sylab->setDownloadable();
       }
     }
  }
