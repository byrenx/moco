...
function loadStudentCourse(ctype){
 window.location = "../../Student_Module/pages/Offered_Courses.php?course="+ctype;
}
function nestPage(pageNum){
 window.location = "../../Studenmt_Module/pages/Offered_Courses.php?course="+pageNum;
}
function selection(searchType,keyWord){
     window.location = "../../Student_Module/pages/Offered_courses.php?course="+stype+"&box"+params;
}

function quizAction(quiz_id){
    var c = confirm("If you press ok this quiz will start.\n Are you sure to take this quiz?");
  alert(quiz_id);
 alert(c);
  if(c==false){
      window.location = "../../Student_Module/pages/Quiz.php";   
  }else{
...
function showEnrollDg(popup_bg,popup_div,formID){
  var course =  $('#'+formID+' #Course_cor').val();
  var how = $('#'+formID+' #mode').val();
...
function changeValue(inputID,formID){
    var inputText = $('#frm_chid #'+inputID).val();    
    $('#frm_chid #'+formID).val(inputText);
}
...
function showErrorDg(popup_bg,popup_div,formID){
  
  var course = document.getElementById('Course_cor').value;
 
 $('#pd-header-title').text('You are Currently Enrolled In: ')
 $('#course').text(course);
 alert('You are Already Enrolled in: '+course);
 //showpopup_form(popup_bg,popup_div);
 //hide_form('CoursekeyCont');
...
function showUnEnrollDg(popup_bg,popup_div,formID){
  ...
  var courseN = $('#'+formID+' #Co_id').val();
  var courseID = $('#'+formID+' #ic_id').val();
  var SCK = $('#'+formID+' #mode').val();
    $('#UN #scr_id').val(courseID);
    
    $('#Prompt').addClass("alert alert-danger");
    $('#DashCourse').text(courseN+"?");
  showpopup_form(popup_bg,popup_div);
}
...
function viewRating(scr_id,assign_id){
    
    var url = "../../libraries/php/executeAction.php";
    var server = "../../";
    var inputs = {"action" : 'update_ass',
		  "target" : 'assignment',
		  "assign_id" : assign_id,
		  "scr_id" : scr_id
                 };
    
    $.get(url,inputs, function(rs){
