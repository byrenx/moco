    ...
    <div  style='position:absolute;top:0px;left: -100px;'>
      <label>
       <?php
	    ...
	    || Search by:
	    <select class="form-control" name="type" id="searchType" class="form-inline">
	    <option value="1">Course Code</option>
	    <option value="3">Instructor</option>
	    <option value="4">Course Title</option>
	    </select>
	    <input class="from-control" name="box" id="keyWord" type="text">
	    <button type="submit"
	    onsubmit="javascript:selection('#searchType','#keyWord')">
...
foreach($offered_list as $offered){
  ...
  if($offered['course_type']==1){
    ...
    if($student->isEnrolledExist($_SESSION['stud_id'],$offered['ic_id'])){
      echo "<td>".$offered['course_desc']."</td>";   
      echo "<td> Open and Online Online</td>";
      echo "<td>".$Start->format('D,M d Y')."</td>";
      echo "<td>".$End->format('D,M d Y')."</td>";
      echo "<td> <b> Already Enrolled </b></td>";
    }else{
